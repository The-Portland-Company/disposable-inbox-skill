...
#
# Agent attribution: every mailbox records which agent created it. This is the
# LABEL of the API key (set once, when you mint the key in the dashboard / via
# POST /v1/api-keys) — it is stamped server-side and cannot be set per request.
# Name the key per agent if you want distinct attribution.
#
